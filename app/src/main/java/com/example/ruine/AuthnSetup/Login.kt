  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    binding.google.setOnClickListener {
      ...
      lifecycleScope.launch {
        try {
          val result = credentialManager.getCredential(request = request, context = this@Login)
          ...
          when {
            googleIdToken != null -> {
              ...
              //                            auth=Firebase.auth
              auth.signInWithCredential(fireBaseCredential).addOnCompleteListener(this@Login) { task
                ->
                if (task.isSuccessful) {
          ...
          Toast.makeText(this@Login, "Credential Failure!! error: ${e.message}", Toast.LENGTH_SHORT)
                  .show()
